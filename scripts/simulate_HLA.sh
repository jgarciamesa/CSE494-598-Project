...
do
	...
	do
		...
		do
			...
			length=$(wc -l data/fasta/id_${stem} | egrep -o '^[^ ]+')
			...
			pos=$(grep -n $id data/fasta/${stem} | grep -o -E ^[0-9]+)  # position
			posp1=$(grep -n $bound data/fasta/${stem} | grep -o -E ^[0-9]+) # position of next
			pos2=$(($posp1 - 1))
